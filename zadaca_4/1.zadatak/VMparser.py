class Parser:
    ...
    def _getLabels(self):
      for (line, n) in self._lines:
          ...
          if len(newLine) == 0 or len(newLine[0]) == 0:
              continue
          if newLine[0] == "label" and len(newLine) == 2:
              self._labels.append(newLine[1])
        ...
        if l[0] == "push":
            ...
            if l[0] == "label" and len(l) == 2:
                ...
                if l[1] not in self._labels:
                    self._flag = False
                    Parser._error("goto", n, "Undefined label")
                    return ""
                return "//" + " ".join(l) + "\n" + self._goto(l[1], n)
            elif l[0] == "if-goto" and len(l) == 2:
                if l[1] not in self._labels:
                    self._flag = False
                    Parser._error("if-goto", n, "Undefined label")
                    return ""
                return "//" + " ".join(l) + "\n" + self._ifgoto(l[1], n)
